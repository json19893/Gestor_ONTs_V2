...
import totalplay.snmp.com.negocio.service.IpoleoLastDownCauseService;

import totalplay.snmp.com.persistencia.repositorio.IpoleoMetricaRepositorio;

@Service
public class poleoLastDownCauseServiceImpl extends constantes implements IpoleoLastDownCauseService {
	...
	@Override
	@Async("taskExecutor")
	public CompletableFuture<responseDto> getLastDownCause(configuracioDto configuracion, String idMonitoreoOlt,
		...
		try {
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					EN_PROCESO_DESC + POLEO + EJECUCION_LAST_DOWN_CASE, fechaInicio.toString(), "", CURSO, configuracion.getIdOlt());
			...
			ejecucionDto proces = utl
					.ejecutaComando(configuracion.getComando() + SPACE + "hwGponDeviceOntControlLastDownCause");

			if(proces.getBuffer() != null) {
				metricas = utl.getMetricasBypoleo(proces, 2, configuracion.getIdOlt(), IdMonitoreoEjecucion);
				...
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_LAST_DOWN_CASE + FINAL_EXITO, fechaInicio,
						LocalDateTime.now().toString(), TERMINO_EXITOSO, configuracion.getIdOlt());
				response.setSms("Exito al descubir memoria");
				response.setCod(0);
			}else {
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_LAST_DOWN_CASE + FINAL_ERROR , fechaInicio,
			...
			response.setCod(1);
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					PROCESO + POLEO + EJECUCION_LAST_DOWN_CASE + FINAL_ERROR + " : " + e, fechaInicio,
					LocalDateTime.now().toString(), TERMINO_ERRO, configuracion.getIdOlt());
