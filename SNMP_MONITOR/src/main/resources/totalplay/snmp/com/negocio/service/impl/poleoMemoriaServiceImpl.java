...
import totalplay.snmp.com.negocio.service.IpoleoMemoriaService;

import totalplay.snmp.com.persistencia.repositorio.IpoleoMetricaRepositorio;

@Service
public class poleoMemoriaServiceImpl extends constantes implements IpoleoMemoriaService {
	...
	/*
	 * Metodo para ejecutar el poleo de las Memoria
	 */
	@Override
	@Async("taskExecutor")
	public CompletableFuture<responseDto> getMetricaMemory(configuracioDto configuracion, String idMonitoreoOlt,
		...
		try {
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					EN_PROCESO_DESC + POLEO + EJECUCION_MEMORY, fechaInicio.toString(), "", CURSO, configuracion.getIdOlt());
			...
			ejecucionDto proces = utl.ejecutaComando(configuracion.getComando() + SPACE + MEMORY);
			
			if(proces.getBuffer() != null) {
				metricas = utl.getMetricasBypoleo(proces, 13, configuracion.getIdOlt(),IdMonitoreoEjecucion);
	
				proces.getProceso().waitFor();
				System.out.println("valor del proceso: " + proces.getProceso().exitValue());
				proces.getProceso().destroy();
				memoriaRepositorio.saveAll(metricas);
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_MEMORY + FINAL_EXITO, fechaInicio, LocalDateTime.now().toString(),
						TERMINO_EXITOSO, configuracion.getIdOlt());
				response.setSms("Exito al descubir memoria");
				response.setCod(0);
			}else {
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_MEMORY + FINAL_ERROR , fechaInicio,
			...
			response.setCod(1);
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					PROCESO + POLEO + EJECUCION_MEMORY + FINAL_ERROR + " : " + e, fechaInicio,
					LocalDateTime.now().toString(), TERMINO_ERRO, configuracion.getIdOlt());
