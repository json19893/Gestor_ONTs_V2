...
import totalplay.snmp.com.negocio.service.IpoleoUpBytesService;

import totalplay.snmp.com.persistencia.repositorio.IpoleoMetricaRepositorio;

@Service
public class poleoUpBytesServiceImpl extends constantes implements IpoleoUpBytesService {
	...
	@Override
	@Async("taskExecutor")
	public CompletableFuture<responseDto> getUpBytes(configuracioDto configuracion, String idMonitoreoOlt,
		...
		try {
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					EN_PROCESO_DESC + POLEO + EJECUCION_UP_BYTES, fechaInicio.toString(), "", CURSO, configuracion.getIdOlt());
			...
			ejecucionDto proces = utl.ejecutaComando(configuracion.getComando() + SPACE + "hwGponOntStatisticUpBytes");

			if(proces.getBuffer() != null) {
				metricas = utl.getMetricasBypoleo(proces, 5, configuracion.getIdOlt(),IdMonitoreoEjecucion);
				...
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_UP_BYTES + FINAL_EXITO, fechaInicio, LocalDateTime.now().toString(),
						TERMINO_EXITOSO, configuracion.getIdOlt());
				response.setSms("Exito al descubir memoria");
				response.setCod(0);
			}else {
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_UP_BYTES + FINAL_ERROR , fechaInicio,
			...
			response.setCod(1);
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					PROCESO + POLEO + EJECUCION_UP_BYTES + FINAL_ERROR + " : " + e, fechaInicio,
					LocalDateTime.now().toString(), TERMINO_ERRO, configuracion.getIdOlt());
