...
import totalplay.snmp.com.negocio.service.IpoleoDownPacketsService;

import totalplay.snmp.com.persistencia.repositorio.IpoleoMetricaRepositorio;

@Service
public class poleoDownPacketsServiceImpl extends constantes implements IpoleoDownPacketsService {
	...
	@Override
	@Async("taskExecutor")
	public CompletableFuture<responseDto> getDownPackets(configuracioDto configuracion, String idMonitoreoOlt,
		...
		try {
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					EN_PROCESO_DESC + POLEO + EJECUCION_DOWN_PACKETS, fechaInicio.toString(), "", CURSO, configuracion.getIdOlt());
			...
			ejecucionDto proces = utl
					.ejecutaComando(configuracion.getComando() + SPACE + "hwGponOntStatisticDownPackts");

			if(proces.getBuffer() != null) {
				metricas = utl.getMetricasBypoleo(proces, 9, configuracion.getIdOlt(), IdMonitoreoEjecucion);
				...
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_DOWN_PACKETS + FINAL_EXITO, fechaInicio, LocalDateTime.now().toString(),
						TERMINO_EXITOSO, configuracion.getIdOlt());
				response.setSms("Exito al descubir memoria");
				response.setCod(0);
			}else {
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_DOWN_PACKETS + FINAL_ERROR , fechaInicio,
			...
			response.setCod(1);
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					PROCESO + POLEO + EJECUCION_DOWN_PACKETS + FINAL_ERROR + " : " + e, fechaInicio,
					LocalDateTime.now().toString(), TERMINO_ERRO, configuracion.getIdOlt());
