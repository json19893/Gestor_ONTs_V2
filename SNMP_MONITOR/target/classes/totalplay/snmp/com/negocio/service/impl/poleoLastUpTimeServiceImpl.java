...
import totalplay.snmp.com.negocio.service.IpoleoLastUpTimeService;

import totalplay.snmp.com.persistencia.repositorio.IpoleoMetricaRepositorio;

@Service
public class poleoLastUpTimeServiceImpl extends constantes implements IpoleoLastUpTimeService {
	...
	@Override
	@Async("taskExecutor")
	public CompletableFuture<responseDto> getLastUpTime(configuracioDto configuracion, String idMonitoreoOlt,
		...
		try {
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					EN_PROCESO_DESC + POLEO + EJECUCION_LAST_UP_TIME, fechaInicio.toString(), "", CURSO, configuracion.getIdOlt());
			...
			ejecucionDto proces = utl
					.ejecutaComando(configuracion.getComando() + SPACE + "hwGponDeviceOntControlLastUpTime");

			if(proces.getBuffer() != null) {
				metricas = utl.getMetricasBypoleo(proces, 3, configuracion.getIdOlt(), IdMonitoreoEjecucion);
				...
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_LAST_UP_TIME + FINAL_EXITO, fechaInicio, LocalDateTime.now().toString(),
						TERMINO_EXITOSO, configuracion.getIdOlt());
				response.setSms("Exito al descubir memoria");
				response.setCod(0);
			}else {
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_LAST_UP_TIME + FINAL_ERROR , fechaInicio,
			...
			response.setCod(1);
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					PROCESO + POLEO + EJECUCION_LAST_UP_TIME + FINAL_ERROR + " : " + e, fechaInicio,
					LocalDateTime.now().toString(), TERMINO_ERRO, configuracion.getIdOlt());
