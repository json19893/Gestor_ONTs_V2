...
import totalplay.snmp.com.negocio.service.IpoleoRunStatusService;

import totalplay.snmp.com.persistencia.repositorio.IpoleoMetricaRepositorio;

@Service
public class poleoRunStatusServiceImpl extends constantes implements IpoleoRunStatusService {
	...
	/*
	 * Metodo para ejecutar el poleo de Run Status
	 */
	@Override
	@Async("taskExecutor")
	public CompletableFuture<responseDto> getRunStatus(configuracioDto configuracion, String idMonitoreoOlt,
		...
		try {
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					EN_PROCESO_DESC + POLEO + EJECUCION_RUN_STATUS, fechaInicio.toString(), "", CURSO, configuracion.getIdOlt());
			...
			ejecucionDto proces = utl
					.ejecutaComando(configuracion.getComando() + SPACE + "hwGponDeviceOntControlRunStatus");
			
			if(proces.getBuffer() != null) {
				metricas = utl.getMetricasBypoleo(proces, 1, configuracion.getIdOlt(),IdMonitoreoEjecucion);
				...
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_RUN_STATUS + FINAL_EXITO, fechaInicio, LocalDateTime.now().toString(),
						TERMINO_EXITOSO, configuracion.getIdOlt());
				response.setSms("Exito al descubir memoria");
				response.setCod(0);
			}else {
				monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
						PROCESO + POLEO + EJECUCION_RUN_STATUS + FINAL_ERROR , fechaInicio,
			...
			response.setCod(1);
			monitOlt.updateMonitorOlt(idRegion, idMonitoreoOlt,
					PROCESO + POLEO + EJECUCION_RUN_STATUS + FINAL_ERROR + " : " + e, fechaInicio,
					LocalDateTime.now().toString(), TERMINO_ERRO, configuracion.getIdOlt());
