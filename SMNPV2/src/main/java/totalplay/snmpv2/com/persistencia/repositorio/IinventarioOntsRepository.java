...
@Aggregation(pipeline = {
	  ...
	  + " }\n"
	  , " {$unset: ['_id', \"ont.olt\"]}"})
List<OntsConfiguracionDto> findOntsEmpresarialesEstatus();

	@Aggregation(pipeline = {
		  "{$match:{tipo:'E'}}\n"
		, "{\n"
		+ "		'$lookup':{\n"
		+ "			from: 'tb_cadenas_onts_empresariales',\n"
		+ "			localField:'numero_serie',\n"
		+ "			foreignField:'numero_serie',\n"
		+ "			as: 'onts',\n"
		+ "		}\n"
		...
		+ "		}\n"
		+ "}\n"
		, "{$unwind:{ path:'$olts',  preserveNullAndEmptyArrays: true} }\n"
		, "{$unwind:{ path:'$onts',  preserveNullAndEmptyArrays: true} }   \n"
		, "{$match:{$or:[{$expr:{$ne:['$olts.id_configuracion', '$onts.configuracion']}}\n"
		+ "                  {$expr:{$ne:['$id_olt', '$onts.id_olt']}}, \n"
		+ "                  {$expr:{$ne:['$tecnologia', '$onts.tecnologia']}},\n"
		+ "                  {onts:[]}\n"
		+ " ]}}\n"
		, " {\n"
		+ "          $lookup: {\n"
		...
		+ "            $project: {\n"
		+ "              ont:'$$ROOT',\n"
		+ "              configuracion: '$olts.configuracion',\n"
		+ "              ip: '$olts.ip',\n"
		+ "              tecnologia: '$olts.tecnologia',\n"
		+ "              id_configuracion: '$olts.id_configuracion',\n"
		+ "            },\n"
		+ "}\n"
